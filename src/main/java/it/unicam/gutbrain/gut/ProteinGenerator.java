...
public class ProteinGenerator implements Runnable {
    ...
    public void run() {
        ...
        while (true) {
            Thread.sleep(Math.abs(random.nextLong() % 600));
    ...
    private void extracted(ProteinType proteinType, String message) throws InterruptedException {
        ...
        if ((int) protein[3] < 50)
            space.put(protein[0], protein[1], protein[2], ((int) protein[3] + 120) * 2);
        else
            space.put(protein[0], protein[1], protein[2], ((int) protein[3] + 40) * 2);
        logger.info(message);
    }
}
